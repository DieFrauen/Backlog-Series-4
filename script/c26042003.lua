...
function c26042003.disop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if re:GetActivateLocation()==LOCATION_HAND and rc:IsType(TYPE_MONSTER) and not rc:IsType(TYPE_RITUAL) and rp~=tp and Duel.NegateEffect(ev) then
		Duel.Hint(HINT_CARD,rp,26042003)
		local g=Duel.GetMatchingGroup(Card.IsDestructable,rp,LOCATION_HAND,0,nil):RandomSelect(tp,1)
		if #g>0 then
